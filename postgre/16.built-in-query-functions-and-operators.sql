...
SELECT 
  EXTRACT(DOW FROM last_checkin) AS month,
  last_checkin
FROM memberships;
...
-- interval of dates
SELECT NOW() - membership_end FROM memberships;

-- * ANOTHER LOOK AT INTERVALS
-- using plus operator work
...
SELECT first_name FROM memberships WHERE first_name LIKE 'J___';
-- ILIKE works in postgre
SELECT first_name FROM memberships WHERE first_name ILIKE 'm%';
...
-- Todo: make the weekday numbers to become weekdays (literal) POSTGRES
-- USUAL CODE 
SELECT 
    TO_CHAR(last_checkin, 'Day') AS weekday,  -- 👈 ito ang maglalabas ng literal na weekday
    last_checkin
FROM memberships;


-- 0 = Monday
SELECT (EXTRACT(ISODOW  FROM last_checkin) - 1) AS weekday, last_checkin FROM memberships;


-- using CASE and WHEN sa conditions
SELECT 
  CASE EXTRACT(ISODOW FROM last_checkin)
  WHEN 1 THEN 'Monday'
  WHEN 2 THEN 'Tuesday'
